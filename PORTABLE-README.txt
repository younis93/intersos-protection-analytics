...
Privacy
This package contains no operational case data. Selected Legal Platform CSV files are processed only on this computer. The application listens only on 127.0.0.1 and is not accessible from other devices on the network.

To end the session
Close the native application window. This stops the local service and clears the current uploaded workbook.
