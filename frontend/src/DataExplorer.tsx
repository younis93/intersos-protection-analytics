...
export default function DataExplorer({metadata}:{metadata:Metadata}){
  ...
  const download=async(format:'csv'|'xlsx')=>{setExporting(format);setError('');try{await exportExplorer(format,{...query,page:1})}catch(e:any){setError(e.message)}finally{setExporting('')}};
  ...
  return <div className="explorer-page">
    ...
    <section className="explorer-controls glass">
      <div className="explorer-main-controls">
        ...
        <button className="soft" onClick={clear} disabled={!searchInput&&!filters.length&&!sortColumn}><RotateCcw/>Clear</button>
      </div>
    ...
    <section className="explorer-table-card glass">
      <header><div><h3>{sheet.name}</h3><p>{formatNumber(result?.matchedRows||0)} matched of {formatNumber(result?.totalRows||sheet.rows)} rows · {visible.length} of {sheet.columns.length} columns</p></div><div className="explorer-export"><button className="soft" onClick={()=>download('csv')} disabled={Boolean(exporting)}><Download/>{exporting==='csv'?'Exporting…':'CSV'}</button><button className="soft" onClick={()=>download('xlsx')} disabled={Boolean(exporting)}><Download/>{exporting==='xlsx'?'Exporting…':'Excel'}</button></div></header>
      <div className={`explorer-table-wrap ${busy?'busy':''}`}><table><thead><tr>{visible.map(column=><th key={column}><button onClick={()=>sort(column)} title="Sort column">{column}{sortColumn===column?<span>{sortDirection==='asc'?' ↑':' ↓'}</span>:null}</button></th>)}</tr></thead><tbody>{result?.rows.map((row,index)=><tr key={`${page}-${index}`}>{visible.map(column=><td key={column} title={display(row[column])}>{display(row[column])}</td>)}</tr>)}</tbody></table>{busy&&<div className="explorer-busy">Loading…</div>}{!busy&&result?.rows.length===0&&<div className="explorer-no-results">No rows match the current search and filters.</div>}</div>
