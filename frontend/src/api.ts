...
export const getDashboard=(page:string,filters:Filters,measure:string)=>fetch(`${API}/dashboard/${page}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({filters,measure,defaultYtd:true})}).then(parse<Dashboard>);
...
export const exportUrl=(page:string,filters:Filters)=>`${API}/export/${page}?filters=${encodeURIComponent(JSON.stringify(filters))}`;
export const getStudio=(page:string,rowDimension:string,columnDimension:string,filters:Filters,measure:string)=>fetch(`${API}/studio`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({page,rowDimension,columnDimension:columnDimension||null,filters,measure,defaultYtd:true})}).then(parse<StudioResult>);
