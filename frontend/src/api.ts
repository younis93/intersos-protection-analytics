...
export const createDuplicateExclusionsBulk = (records: Array<Pick<DuplicateExclusion, "caseId" | "rule" | "name" | "project" | "source"> & Partial<Pick<DuplicateExclusion,"dataset"|"identifierType"|"identifierValue">>>) =>
  fetch(`${API}/legal/duplicate-exclusions/bulk`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ records }) }).then(parse<{created:number; duplicates:number; rows: DuplicateExclusion[]; count: number}>);
