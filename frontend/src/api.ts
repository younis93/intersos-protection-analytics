...
export const getStudio=(page:string,rowDimension:string,columnDimension:string,filters:Filters,measure:string,signal?:AbortSignal)=>fetch(`${API}/studio`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({page,rowDimension,columnDimension:columnDimension||null,filters,measure,defaultYtd:true}),signal}).then(parse<StudioResult>);
