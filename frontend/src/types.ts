...
export interface Metadata {ready:boolean;source:string|null;loadedAt:string|null;pages:Record<string,{rows:number;filters:Record<string,string[]>;dimensions?:string[]}>;dataExplorer?:{sheets:ExplorerSheet[];warnings:string[]}}
...
export interface LegalOverview {beneficiaries:number;assessments:number;services:number;followups:number|null;fees:number|null;awareness:number|null;deportations:number|null;lawyers:number;totalFlags:number;severity:Record<string,number>;rules:Record<string,number>;charts?:{assessmentsByLocation:LegalOverviewChart[];assessmentsByLawyer:LegalOverviewChart[];assessmentStatus:LegalOverviewChart[];representationServiceStatus:LegalOverviewChart[]};representationCompletionRate?:number;activityTrend?:{month:string;assessments:number;services:number}[];locationPerformance?:{location:string;assessments:number;representationServices:number;detained:number;released:number;completionRate:number}[];insight?:string;dataQuality?:LegalOverviewChart[];deportationsByGovernorate?:LegalOverviewChart[];detention2026?:{trend:{month:string;detainedAssessments:number;released:number}[];map:{label:string;count:number;detained:number;released:number;values:string[]}[]}}
export interface LegalMetadata {ready:boolean;source:string|null;warnings:string[];availability:Record<string,boolean>;sheets:LegalSheet[];months:string[];reviewCounts:Record<string,number>;overview?:LegalOverview}
...
export interface IndicatorReportGroup {id:"all"|"refugee"|"idp";label:string;indicators:IndicatorReportItem[]}
export interface IndicatorReport {fromDate:string;toDate:string;ageGroups:string[];filterOptions:{projects:string[];locations:string[];locationsByProject:Record<string,string[]>;years:string[];quarters:string[];months:string[]};activeFilters:{projects:string[];locations:string[];years:string[];quarters:string[];months:string[]};groups:IndicatorReportGroup[]}
export interface LegalFlag {dataset:string;rule:string;severity:string;row:number;recordId:string;name:string;caseId:string;assessmentId:string;serviceId:string;detail:string;action:string;lawyer:string;project:string;location:string;phone:string;dateOfBirth:string;awarenessId:string;sessionTopic:string;duplicateGroup?:string;nameMatchMode?:"exact"|"variation";duplicateSimilarity?:number;spouseName?:string;spouseDateOfBirth?:string;spouseAge?:number|null;maritalStatus?:string}
export interface LegalReview {dataset:string;total:number;page:number;pageSize:number;rules:string[];ruleCounts:Record<string,number>;filterOptions:Record<'severity'|'lawyer'|'project'|'location',string[]>;availableMonths:string[];activeComparisonMonth:string;nameRecordCount:number;eligibleNameRecordCount:number;nameCompareCharsApplied:number;allowNameVariationsApplied:boolean;rows:LegalFlag[]}
