...
export interface Row {label:string;count:number;percent:number;filterValue?:string}
...
export interface UpdateCheck {enabled:boolean;currentVersion:string;available:boolean;latestVersion?:string;notes?:string;publishedAt?:string;sizeBytes?:number;message?:string}
export interface UpdateStatus {phase:'idle'|'downloading'|'verifying'|'installing'|'restarting'|'error';progress:number;error?:string|null;downloadedBytes?:number;totalBytes?:number;currentVersion:string}
...
export interface LegalMetadata {ready:boolean;revision?:string;loading?:boolean;source:string|null;warnings:string[];availability:Record<string,boolean>;features?:{awareness?:boolean;detention:boolean;deportation:boolean};sheets:LegalSheet[];months:string[];reviewCounts:Record<string,number>;overview?:LegalOverview}
